# Fork Miracle Agent，搭一个你自己的 coding agent

前六篇我们把 Miracle Agent 拆开看了个遍。循环、工具、模型接口、上下文压缩、并发、子 agent、CLI，每个零件都摊在桌上看过了。这一篇我们把它们装回去，而且装成你自己的。
...
```bash
# 在 GitHub 上 fork he-yufeng/Miracle Agent 到你自己名下，然后
git clone https://github.com/<你的用户名>/Miracle Agent
cd Miracle Agent
pip install -e .
...
Miracle Agent 默认 `gpt-5.5`，但你未必想用它。第三篇讲过，换模型就是换环境变量。开发期我强烈建议先用本地的 Ollama，一分钱不花，随便折腾：
...
新建 `miracle_agent/tools/fetch.py`：
...
跑起来，问它「抓一下 https://raw.githubusercontent.com/he-yufeng/Miracle Agent/main/README.md 看看这项目干嘛的」，它会调 `fetch_url` 再讲给你听。
...
agent 的行为风格，不在某段 if-else 里，在系统提示词里。打开 `miracle_agent/prompt.py`，那段 `# Rules` 就是你的 agent 的「行为准则」。原版里有这么几条：
...
Miracle Agent 顶层导出了 `Agent`、`LLM`、`Config`，意味着你不必只能用它那个交互式终端，你可以把它当库，搭出一个完全不同形态的 agent。
...
report = reviewer.chat(
    "审查 miracle_agent/agent.py，找出并发相关的隐患，列成一份清单。"
)
print(report)
```
...
Miracle Agent 是个起点，不是终点。它故意留白了不少地方，每一处都是你可以往下做的方向，而且前面几篇基本都点过名：

- **给 bash 上真沙箱**。第二篇说透了，正则黑名单只是防手滑，不是安全边界。要面对不可信输入，得上 `seccomp` 或者容器级隔离。
- **补 fallback 模型和美元硬预算**。第三篇讲过 Miracle Agent 故意没做这两样，因为它们会拖进 provider 专属逻辑。你要做生产部署，这两样迟早得加。
- **把并发做得更细**。第五篇说的那条，区分工具「读」还是「写」来决定能不能并发，Miracle Agent 还没做，是可以认真补的。
- **接 MCP**。让你的 agent 能挂上 Model Context Protocol 的工具生态，一下子接通一大批现成的外部能力。
- **给子 agent 更多模式**。第五篇提过 Claude Code 的子 agent 能在独立 worktree 或后台跑，Miracle Agent 只做了最朴素的同步一种。
